...
function Adios() {
  ...
  const onSubmit = async (form: FieldValues) => {
    ...
    if (form.name.trim() !== name)
      return setSubmitError("the name doesn't match");
    try {
      await deleteACC(form);
      navigate("/login");
    } catch (error) {
      setSubmitError(
        ((error as AxiosError).response?.data as APIERROR).message
      );
    }
  };
    ...
    <main className="bg-jira-gradient grid w-full place-items-center">
      <div className="w-11/12 max-w-[25rem] rounded-md bg-white px-8 pt-12 pb-14">
        ...
        >
          ...
          <div>
            <span>
              Please type "<span className="text-blue-500">{name}</span>" to
              confirm
            </span>
            <InputWithValidation
              label=""
